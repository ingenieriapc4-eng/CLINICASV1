...
# ---------------------------------------------------------------------------
# Código de activación para compatibilidad / pruebas.
# En la versión ONLINE el acceso inicial es DEMO y la activación se realiza
# después desde la pantalla de activación. En la versión LOCAL se conserva el
# flujo anterior de activación durante la configuración inicial.
#
...
def _license_from_settings():
    ...
    if stored_token:
        try:
            ...
            if payload.get("product") != LICENSE_PRODUCT:
                status = "invalida"
            elif (not ONLINE_MODE) and payload.get("installation_id") and payload.get("installation_id") != installation_id:
                status = "invalida"
            elif (not ONLINE_MODE) and payload.get("machine_binding") and payload.get("machine_binding") != machine_fp:
                status = "invalida"
        except Exception:
            status = "invalida"
    ...
    return {
        ...
        "machineBinding": "" if ONLINE_MODE else machine_fp[:12].upper(),
...
def _seed_online_demo(db):
    ...
    if not ONLINE_MODE:
        return
    real_users = db.execute("SELECT COUNT(*) AS c FROM users WHERE COALESCE(role, '') != 'demo'").fetchone()["c"]
    if real_users:
        return
    demo = db.execute("SELECT id FROM users WHERE id = ? LIMIT 1", (ONLINE_DEMO_USER_ID,)).fetchone()
    if not demo:
        demo_password = generate_password_hash(uuid.uuid4().hex)
        db.execute(
            "INSERT INTO users (id, username, password_hash, role, permissions, display_name, active, must_change_password, created_at) VALUES (?, ?, ?, 'demo', 'all', ?, 1, 0, ?)",
            (ONLINE_DEMO_USER_ID, "demo", demo_password, "Demostración", datetime.now().isoformat(timespec="seconds")),
        )
        accepted_at = datetime.now().isoformat(timespec="seconds")
        db.execute(
            "INSERT INTO legal_acceptances (id, user_id, username, terms_version, accepted_at) VALUES (?, ?, ?, ?, ?)",
            (uuid.uuid4().hex, ONLINE_DEMO_USER_ID, "demo", CURRENT_TERMS_VERSION, accepted_at),
        )

    patient_id = "demo-patient-1"
    now = datetime.now().isoformat(timespec="seconds")
    today = datetime.now().strftime("%Y-%m-%d")
    if not db.execute("SELECT 1 FROM patients WHERE id = ? LIMIT 1", (patient_id,)).fetchone():
        db.execute(
            "INSERT INTO patients (id, name, dob, phone, email, address, allergies, history, notes, photo, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (patient_id, "Paciente de demostración", "1988-05-20", "7000-0000", "demo@clinicasv.local", "Dato de ejemplo", "Ninguna registrada", "Antecedentes ficticios para demostración.", "Registro de prueba. No corresponde a una persona real.", "", now, now),
        )
    if not db.execute("SELECT 1 FROM appointments WHERE id = ? LIMIT 1", ("demo-appointment-1",)).fetchone():
        db.execute(
            "INSERT INTO appointments (id, patient_id, date, time, duration, status, reason, notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ("demo-appointment-1", patient_id, today, "10:00", 30, "pendiente", "Consulta general", "Cita de demostración.", now, now),
        )
    if not db.execute("SELECT 1 FROM clinical_records WHERE id = ? LIMIT 1", ("demo-record-1",)).fetchone():
        # V6.7.3 FIX: clinical_records tiene 20 columnas aquí; deben existir 20 placeholders.
        db.execute(
            "INSERT INTO clinical_records (id, patient_id, fecha, especialidad, motivo, presion, temperatura, pulso, peso, talla, saturacion, examen, diagnostico, medicamentos, indicaciones, seguimiento, created_at, tratamiento, costo, especialidad_datos) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ("demo-record-1", patient_id, today, "Medicina general", "Consulta de demostración", "120/80", "36.7", "72", "70", "1.70", "98", "Exploración ficticia de demostración.", "Diagnóstico de ejemplo", "Medicamento de ejemplo", "Indicaciones ficticias para demostración.", "Seguimiento en 7 días", now, "Tratamiento de ejemplo", 25, "{}"),
        )
    if not db.execute("SELECT 1 FROM prescriptions WHERE patient_id = ? LIMIT 1", (patient_id,)).fetchone():
        db.execute(
            "INSERT INTO prescriptions (patient_id, medications, instructions, updated_at) VALUES (?, ?, ?, ?)",
            (patient_id, "[{\"nombre\":\"Medicamento de ejemplo\",\"presentacion\":\"Tableta\",\"via\":\"Oral\",\"dosis\":\"1\",\"frecuencia\":\"Cada 12 horas\",\"duracion\":\"5 días\"}]", "Receta ficticia para demostración. No usar como indicación médica.", now),
        )
    if not db.execute("SELECT 1 FROM charges WHERE id = ? LIMIT 1", ("demo-charge-1",)).fetchone():
        db.execute(
            "INSERT INTO charges (id, patient_id, date, treatment, cost, paid, notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ("demo-charge-1", patient_id, today, "Consulta de demostración", 25.0, 0.0, "Cargo ficticio", now, now),
        )
    budget_id = "demo-budget-1"
    if not db.execute("SELECT 1 FROM budgets WHERE id = ? LIMIT 1", (budget_id,)).fetchone():
        db.execute(
            "INSERT INTO budgets (id, patient_id, title, status, notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (budget_id, patient_id, "Presupuesto de demostración", "pendiente", "Presupuesto ficticio.", now, now),
        )
    if not db.execute("SELECT 1 FROM budget_items WHERE id = ? LIMIT 1", ("demo-budget-item-1",)).fetchone():
        db.execute(
            "INSERT INTO budget_items (id, budget_id, description, cost) VALUES (?, ?, ?, ?)",
            ("demo-budget-item-1", budget_id, "Consulta de demostración", 25.0),
        )


def _remove_online_demo(db):
    """Elimina únicamente los datos ficticios cuando una instalación online se activa."""
    if not ONLINE_MODE:
        return
    demo_ids = ["demo-patient-1"]
    db.execute("DELETE FROM budget_payments WHERE budget_id = ?", ("demo-budget-1",))
    db.execute("DELETE FROM budget_items WHERE budget_id = ?", ("demo-budget-1",))
    db.execute("DELETE FROM budgets WHERE id = ?", ("demo-budget-1",))
    db.execute("DELETE FROM prescriptions WHERE patient_id = ?", ("demo-patient-1",))
    db.execute("DELETE FROM clinical_records WHERE patient_id = ?", ("demo-patient-1",))
    db.execute("DELETE FROM charges WHERE patient_id = ?", ("demo-patient-1",))
    db.execute("DELETE FROM appointments WHERE patient_id = ?", ("demo-patient-1",))
    db.execute("DELETE FROM documents WHERE patient_id = ?", ("demo-patient-1",))
    for pid in demo_ids:
        db.execute("DELETE FROM patients WHERE id = ?", (pid,))
    db.execute("DELETE FROM legal_acceptances WHERE user_id = ?", (ONLINE_DEMO_USER_ID,))
...
def any_user_exists():
    db = get_db()
    row = db.execute("SELECT COUNT(*) AS c FROM users WHERE COALESCE(role, '') != 'demo'").fetchone()
    return row["c"] > 0
...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not is_demo and request.endpoint not in ("legal_acceptance", "logout") and not _terms_accepted_for_current_user(session.get("user_id")):
        ...
        if not is_demo and session.get("must_change_password") and request.endpoint not in ("change_password", "force_password_change", "logout", "legal_acceptance"):
            if request.path.startswith("/api/"):
...
@app.before_request
def enforce_setup():
    public_online = {"/", "/demo", "/login", "/setup", "/terms", "/logout"}
    if request.path in public_online or request.path == "/static" or request.path.startswith("/static/"):
        return
    if ONLINE_MODE:
        if not any_user_exists() and not session.get("demo_mode"):
            if request.path.startswith("/api/"):
                return jsonify({"error": "Activa la licencia y crea el usuario administrador primero."}), 403
...
def setup():
    # Entrar aquí desde DEMO significa que se quiere pasar a la activación real.
    ...
    if request.method == "POST":
            ...
            if license_file and license_file.filename:
                ...
                if not filename.lower().endswith(".clsv"):
                    ...
                    try:
                        ...
                        elif (not ONLINE_MODE) and license_payload.get("installation_id") and license_payload.get("installation_id") != expected_installation:
                            error = "La licencia corresponde a otra instalación."
                        elif (not ONLINE_MODE) and license_payload.get("machine_binding") and license_payload.get("machine_binding") != expected_machine:
                ...
                error = "Para activar la versión en línea, importa el archivo .clsv o introduce el código de activación." if ONLINE_MODE else "Importa el archivo de licencia .clsv o introduce un código de activación."
        ...
        if not error:
            ...
            elif ONLINE_MODE and activation_code:
                # El código simple se conserva para pruebas/compatibilidad.
                # Las licencias comerciales firmadas pueden conservar además su vigencia y características.
                code_updates = {
                    "license_id": "ONLINE-CODE",
                    "license_type": "Online (código)",
                    "license_holder": username,
                    "license_issued_at": datetime.now().isoformat(timespec="seconds"),
                    "license_expires_at": "",
                    "license_status": "activa",
                    "license_features": "",
                    "license_token": "",
                }
                for key, value in code_updates.items():
                    db.execute("UPDATE settings SET value = ? WHERE key = ?", (str(value), key))
            db.commit()
            return redirect(url_for("login"))
    return render_template("setup.html", error=error, online_mode=ONLINE_MODE)
...
def activate_license():
    ...
    if (not ONLINE_MODE) and payload.get("installation_id") and payload.get("installation_id") != expected_installation:
        return jsonify({"error": "La licencia corresponde a otra instalación."}), 400
    if (not ONLINE_MODE) and payload.get("machine_binding") and payload.get("machine_binding") != expected_machine:
...
def demo():
    if not ONLINE_MODE:
        return redirect(url_for("login"))
    # La DEMO online es independiente de la cuenta/licencia principal.
    # Aunque ya exista un usuario activado en la base principal, un visitante
    # debe poder entrar a la DEMO desde la pantalla pública.
    if not DEMO_DATABASE_URL:
        return redirect(url_for("home", demo_config_error=1))
    ...
    session["user_id"] = ONLINE_DEMO_USER_ID
...
def login():
    if ONLINE_MODE and not any_user_exists():
        return redirect(url_for("home"))
    if not ONLINE_MODE and not any_user_exists():
        return redirect(url_for("setup"))
